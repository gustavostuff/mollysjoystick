zhlider = (function (d) {
  ...
  let registerEvents = (container, id) => {
    container.onclick = (evt) => {
      changeSlider(container, isLeftHalf(evt));
      clearInterval(intervals[id]);
      intervals[id] = setInterval(() => changeSlider(container), defaultDelay);
    }
  };

  let resetInterval = (container, id) => {
    intervals[id] = setInterval(() => changeSlider(container), defaultDelay);
  };
  ...
  return {
      ...
      if (container) {
        if (container.children.length < 2) {
          ...
          registerEvents(container, id);
          resetInterval(container, id);
